...
- **Singleton-row settings, split across two persistence mechanisms** — `brand_settings`, `theme_settings`, `settings` (COD), and `invoice_settings` are genuine Supabase tables with exactly one row, fetched via `.limit(1).single()` or `.maybeSingle()`. `LoyaltySettings`/shop filter colors are still **`localStorage`-only** (`sd_settings_v1`) — meaning they don't sync across browsers or between admins. (Invoice Template and Brands used to be localStorage too — both fixed to real Supabase tables, `invoice_settings`/`brands`, in `20260812000006`, since the invoice one was a genuine bug: a customer viewing their own invoice on their own device never saw the admin's actual configured brand info, only hardcoded defaults.)
...
Two independent systems on one page: **Categories** (hierarchical, Supabase-backed, one level of nesting only) and **Brands** (flat, Supabase-backed since `20260812000006` — table didn't actually exist before then, see 02-DATABASE-SCHEMA.md). Add/inline-rename/delete both; a category delete is a **soft delete** (`is_active = false`) — existing products keep their now-stale `category_id` FK, they just stop appearing in pickers. Brand "delete" is likewise a soft delete (`is_active = false`); products still reference brands by name in the plain `brand` text field, not a real `brand_id` FK. Renaming a category explicitly preserves its existing `parentId` (a code comment calls this out) so a rename never accidentally un-parents it.
...
A **single shared template** (brand name, tagline, GSTIN, contact, address, accent color, terms, signatory, footer) — a real Supabase singleton (`invoice_settings`) as of `20260812000006`. Previously `localStorage`-only, which was a genuine bug: the customer-facing invoice page read the same key from the *customer's own browser*, so it never actually showed the admin's configured info to a real customer, only to the admin viewing their own device. Fixed — both this page and `/invoice/$id` now read/write the same DB row.
...
2. **Two config surfaces still use `localStorage` instead of Supabase** (Loyalty settings, Shop settings) — doesn't sync across browsers/admins, silently. Invoice Template and Brands were fixed to real Supabase tables (`invoice_settings`, `brands`, both `20260812000006`).
